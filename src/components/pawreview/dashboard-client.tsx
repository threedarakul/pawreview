...
export function DashboardClient({ employees }: { employees: Employee[] }) {
  ...
  function handleSave(data: EmployeeInput) {
    setFormError("");
    ...
    <div className="flex min-h-full flex-col bg-background">
      ...
      <main className="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-6 px-4 py-8 sm:px-6">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <StatCard label="เกินกำหนด" value={counts.overdue} tone="overdue" />
          <StatCard
            label="ใกล้ครบกำหนด"
        ...
        <div className="overflow-hidden rounded-xl border border-border bg-card">
          {visibleEmployees.length === 0 ? (
            <EmptyState
              hasAnyEmployee={employees.length > 0}
              onAdd={openAddForm}
            />
          ) : (
            <Table>
              ...
              <TableBody>
                {visibleEmployees.map((emp) => (
                  <TableRow key={emp.id}>
                    ...
                    <TableCell>
                      {formatThaiDate(new Date(emp.startDate))}
                    </TableCell>
                    <TableCell>
